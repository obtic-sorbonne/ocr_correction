...
def generate_json_with_predictions(dataset, split, output_path="output.json", min_length=15, max_length=1300):
    ...
    """
    data = []
    num_examples = len(dataset[split]["Sentence_OCR"])

    for i in tqdm(range(num_examples), desc=f"Processing {split} examples"):

        ocr_text = dataset[split]["Sentence_OCR"][i]
        ...
        if min_length <= len(ocr_text) <= max_length and min_length <= len(gs_text) <= max_length:
            ...
            dataset_cer = dataset[split]["CER"][i]
            dataset_wer = dataset[split]["WER"][i]
...
generate_json_with_predictions(dataset, 'test', output_path="Llama-3.2-3B-ocr-correction-3-instruction-corrected-mixed-data-real-evaluation.json")
